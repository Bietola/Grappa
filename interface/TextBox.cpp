...
//set text
gr::TextBox& gr::TextBox::setText(const std::string& text) {
    mTextInfo.text = text;
    mNeedToUpdateVertices = true;
	return *this;
}
//add text
gr::TextBox& gr::TextBox::addText(const std::string& text) {
    mTextInfo.text += text;
    mNeedToUpdateVertices = true;
	return *this;
}
    ...
    if(mNeedToUpdateVertices){
        ...
        for(size_t j = 0, vj = 0; j < newText.size(); j++, vj++){
            ...
            //handle letters (non-handled characters are rendered as letters)
                ...
                mVertices[vj*4 + 3]  = sf::Vertex({bRect.left, bRect.top + bRect.height}, color,                {tRect.left, tRect.top + tRect.height});
            }
